-- Fix RLS issue by creating a test user and updating sample data
-- First, let's clean up the existing sample data with invalid user_id
...
-- Insert a test user directly into auth.users (for development/testing only)
...
-- Now insert sample data with the real user ID
...
-- Sample prescriptions with real user ID
